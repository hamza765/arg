$(document).ready(function() {
    ...
    $("#scan_button").on('click', function() {
        ...
        // update address bar
        var slug = "/?url=" + url;
        if (advanced) slug += "&advanced=true";
    ...
    function formatReport(data) {
        ...
        //Cipher Suites
        // $("#ciphersuites").html('<div class="row"><div class="col-xs-1 h4">Priority</div><div class="col-xs-4 h4">Cipher Suite</div><div class="col-xs-3 h4">Protocols</div><div class="col-xs-3 h4">Perfect Forward Secrecy (PFS)</div><div class="col-xs-1 h4">Curves</div></div>');

        // for (var i = 0; i < data.ciphersuites.length; i++) {
        //     var row = '<div class="row">'
        //     row += '<div class="col-xs-1">' + data.ciphersuites[i].priority + '</div>'
        //     row += '<div class="col-xs-4 wordwrap">' + data.ciphersuites[i].ciphersuite
        //     if (data.ciphersuites[i].ciphersuite.match(/^.*SHA$/) || data.ciphersuites[i].ciphersuite.match(/^.*MD5$/)) row += ' <span class="small label label-danger">WHS</span>'
        //     row += '</div>'
        //     row += '<div class="col-xs-3 wordwrap">' + data.ciphersuites[i].protocols + '</div>'
        //     if (data.ciphersuites[i].pfs == "None") {
        //         row += '<div class="col-xs-4 center"><span class="small label label-warning">NFS</span></div>'
        //     } else {
        //         row += '<div class="col-xs-3 wordwrap">' + data.ciphersuites[i].pfs + '</div>'
        //         row += '<div class="col-xs-1">' + data.ciphersuites[i].curves + '</div>'
        //     }
        //     row += '</div>'
        //     $("#ciphersuites").append(row);
        // }
        // $("#ciphersuites").append('<br><div class="row small"><span class="label label-danger">WHS</span> = Weak Hash Algorithm</div>');
        // $("#ciphersuites").append('<div class="row small"><span class="label label-warning">NFS</span> = No Forward Secrecy</div>');

        // //Insecure Links
        var insecure_links_count = data.insecure_links.length;
        var insecure_links_half = Math.floor((insecure_links_count + 1) / 2);



        //Insecure Links Graph
        $("#insecure_links_graph").html('<div class="GaugeMeter' + ' center" data-theme="Green-Gold-Red" data-size="180" data-width="40" data-stripe="4"></div>');
        var default_max = 10;
        var max = (insecure_links_count > default_max) ? insecure_links_count : default_max;
        $(".GaugeMeter").attr("data-total", max);
        $(".GaugeMeter").attr("data-used", insecure_links_count);
        $(".GaugeMeter").attr("data-text", insecure_links_count);
        $(".GaugeMeter").attr("data-percent", (insecure_links_count / max) * 100);
        $(".GaugeMeter").gaugeMeter();

        //Insecure Links List
        $("#insecure_links_left").html('<p>');
        $("#insecure_links_right").html('<p>');
        for (var i = 0; i < insecure_links_half; i++) {
            $("#insecure_links_left").append(data.insecure_links[i] + '<br>');
        }
        $("#insecure_links_left").append('<p>');
        for (var i = insecure_links_half; i < insecure_links_count; i++) {
            $("#insecure_links_right").append(data.insecure_links[i] + '<br>');
        }
        $("#insecure_links_right").append('<p>');
    ...
    function decode(body, csr) {
        ...
        function csrFormat(csrIn) {
            ...
            sans.map(function(curr, i, arr) {
                $('#csr_san').append(arr[i] + (i == arr.length - 1 ? "." : ', '));
            });
